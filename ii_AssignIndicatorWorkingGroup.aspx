<%@ Page Title="" Language="C#" MasterPageFile="~/akshara.master" AutoEventWireup="true" CodeFile="ii_AssignIndicatorWorkingGroup.aspx.cs" Inherits="ii_AssignIndicatorWorkingGroup" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <div class="container">
        <div class="section-title text-center">
            <br />
            <div>
                <h2 class="background double animated wow fadeInUp color1" style="color: #000000;" data-wow-delay="0.2s"><span><strong>Assign Indicator → Working Group</strong></span></h2>
            </div>
        </div>
        <br /><br />

        <asp:Label runat="server" Text="Working Group:" AssociatedControlID="ddlWorkingGroups" />
        <asp:DropDownList runat="server" ID="ddlWorkingGroups" AutoPostBack="true" OnSelectedIndexChanged="ddlWorkingGroups_SelectedIndexChanged" />

        <asp:Label runat="server" Text="Indicator:" AssociatedControlID="ddlIndicators" />
        <asp:DropDownList runat="server" ID="ddlIndicators" />

        <asp:Button runat="server" ID="btnAssign" Text="Assign Mapping" OnClick="btnAssign_Click" />
        ...
        <asp:GridView
            ...
            <Columns>
                <asp:BoundField DataField="MapID" HeaderText="ID" Visible="false" />
                <asp:BoundField DataField="IndicatorID" HeaderText="Indicator ID" Visible="false" />
                <asp:BoundField DataField="WorkingGroupID" HeaderText="Working Group ID" Visible="false" />
                <asp:TemplateField HeaderText="Indicator">
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblIndicatorName" Text='<%# GetIndicatorName(Eval("IndicatorID")) %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Working Group">
                    <ItemTemplate>
                        <asp:Label runat="server" ID="lblWorkingGroupName" Text='<%# GetWorkingGroupName(Eval("WorkingGroupID")) %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="Action">
                    <ItemTemplate>
                        <asp:LinkButton runat="server" CommandName="DeleteRow" CommandArgument='<%# Eval("MapID") %>' Text="Remove" />
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
        </asp:GridView>
