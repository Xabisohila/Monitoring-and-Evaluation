<%@ Page Title="PMTDP Approvals" Language="C#" MasterPageFile="~/akshara.master"
    AutoEventWireup="true" CodeFile="j_PMTDPApprovalList.aspx.cs" Inherits="j_PMTDPApprovalList" %>
...
.page-header { padding: 36px 0 22px; border-bottom: 1px solid var(--border); margin-bottom: 28px; }
.page-header h2 { margin: 0 0 4px; font-size: 24px; font-weight: 800; color: var(--primary); }
...
<div class="container" style="padding-top:80px; padding-bottom:60px; max-width:1100px;">
    ...
    <div class="page-header">
        <h2><i class="glyphicon glyphicon-list-alt"></i> PMTDP Upload Approvals</h2>
        <p>Review pending PMTDP uploads. Only uploads from other users are shown for action.</p>
    </div>
                ...
                <asp:Repeater ID="rptPending" runat="server">
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td><%# Server.HtmlEncode(Eval("SubmittedBy").ToString()) %></td>
                            <td><%# Eval("UploadDate", "{0:dd MMM yyyy HH:mm}") %></td>
                            <td><%# Eval("DataRowCount") %></td>
                            <td><a href='j_PMTDPApprovalView.aspx?id=<%# Eval("UploadRequestID") %>' class="btn-review">Review</a></td>
                        </tr>
                    </ItemTemplate>
    ...
    <%-- Recent decisions --%>
    <div class="section-sep">Recent Decisions</div>
    <div class="form-card">
        <div class="card-header">
            <h4>Reviewed Uploads</h4>
        </div>
        <asp:Panel ID="pnlNoReviewed" runat="server" Visible="false">
            <div class="empty-state">No uploads have been reviewed yet.</div>
        </asp:Panel>
        <asp:Panel ID="pnlReviewed" runat="server" Visible="false">
            <div class="table-scroll" id="reviewedTableWrap">
                <asp:Repeater ID="rptReviewed" runat="server">
                    <HeaderTemplate>
                        <table class="history-tbl">
                        <tr>
                            <th>Ref #</th>
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td><a href='j_PMTDPApprovalView.aspx?id=<%# Eval("UploadRequestID") %>' class="btn-view">View &rarr;</a></td>
                        </tr>
                    </ItemTemplate>
                    <FooterTemplate></table></FooterTemplate>
                </asp:Repeater>
            </div>
            <div id="reviewedPager" style="display:none; padding:12px 20px; border-top:1px solid #edf2f7;
                 display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
                <span id="reviewedPageInfo" style="font-size:12px; color:#64748b;"></span>
                <div style="display:flex; gap:6px;">
                    <button type="button" id="reviewedPrev" onclick="reviewedPage(-1)"
                        style="padding:5px 14px; font-size:12px; font-weight:600; border-radius:6px;
                               border:1px solid #dce3ec; background:#fff; color:#1a2b4a; cursor:pointer;">
                        &lsaquo; Prev
                    </button>
                    <button type="button" id="reviewedNext" onclick="reviewedPage(1)"
                        style="padding:5px 14px; font-size:12px; font-weight:600; border-radius:6px;
                               border:1px solid #dce3ec; background:#fff; color:#1a2b4a; cursor:pointer;">
                        Next &rsaquo;
                    </button>
...
(function () {
    var _pendingPage   = makePager('pendingTableWrap',  'pendingPager',  'pendingPageInfo',  'pendingPrev',  'pendingNext',  10);
    var _reviewedPage  = makePager('reviewedTableWrap', 'reviewedPager', 'reviewedPageInfo', 'reviewedPrev', 'reviewedNext', 10);
    window.pendingPage  = function(d) { if (_pendingPage)  _pendingPage(d);  };
    window.reviewedPage = function(d) { if (_reviewedPage) _reviewedPage(d); };
}());
</script>
</asp:Content>
